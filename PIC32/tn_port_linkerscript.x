/* TNKernel-PIC32Stack linker script */
...
EXTERN (_ramfunc_begin _ramfunc_end)

EXTERN (tn_user_stack_seg_start tn_user_stack_seg_end)
EXTERN (_bmxdudba_address _bmxdupba_address)
...
SECTIONS
{
  ...
  */
  ...
  _end = . ;
  _bss_end = . ;
  /* Reserve space for the linker-allocated heap and stack */
  . += _min_heap_size ;
  . += _min_stack_size ;
  . = ALIGN(1024) ;
  _bmxdkpba_address = (. - ORIGIN(kseg1_data_mem)) ;
  .ramfunc ALIGN(1024) :
  {
    _ramfunc_begin = . ;
    *(.ramfunc*)
  ...
  . = ALIGN(1024) ;
  _bmxdudba_address = (. - ORIGIN(kseg1_data_mem)) ;
  .tn_user_stack ALIGN(1024) :
  {
    tn_user_stack_seg_start = . ;
    *(tn_user_stack*)
    . = ORIGIN(kseg1_data_mem)+LENGTH(kseg1_data_mem) ;
    tn_user_stack_seg_end = . ;
  } > kseg1_data_mem
  . = ALIGN(4) ;
